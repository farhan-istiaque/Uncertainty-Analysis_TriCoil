...
data_file_loc = prj_fold_loc + "\\03-Trimmed Raw Data\\"
...
Unit Conversion
"""
#m_dot_w_avg = m_dot_w_avg/3600 #kg/s

#m_dot_r_avg = m_dot_r_avg/3600 #kg/s

#Pressure

#P_w_in_avg = (P_w_in_avg * 6894.76) + 101325 #Conversion from PSIG to Pa



#P_w_out_avg = (P_w_out_avg * 6894.76) + 101325 #converstion from PSIG to Pa

#P_r_in_avg = (P_r_exv_in_avg * 6894.76) + 101325 #Conversion from PSIG to Pa
#P_r_out_avg = (P_r_evap_out_avg * 6894.76) + 101325 #Conversion from PSIG to Pa

#P_atm_avg = np.mean(P_atm) * 6894.76 #converstion from PSI to Pa
#dp_a_avg = np.mean(dp_a) * 248.84 #conversion from in W.G to Pa
#P_a_in_avg = (np.mean(P_a_in) * 248.84) #conversion from in W.G to Pa

#Temperature concersion from C to K
# T_w_in_avg += 273.15
# T_w_out_avg += 273.15
...
# """
# Change the title of random uncertainty data file here
# """
# rnd_file_loc = prj_fold_loc + "\\05-Random Uncertainty Data\\Random_Error_Hamid.csv"

# df_2 = pd.read_csv(rnd_file_loc)
...
#rnd_un_T_db_in = df_2[["Tdb_in (K)"]].to_numpy()
# rnd_un_T_wb_in = df_2[["Twb_in (K)"]].to_numpy()
# rnd_un_T_db_out = df_2[["Tdb_out (K)"]].to_numpy()
# rnd_un_T_wb_out = df_2[["Twb_out (K)"]].to_numpy()
# rnd_un_dp_a = df_2[["dP_nozzle_TriCoil (Pa)"]].to_numpy()
# rnd_un_Pa_in = df_2[["Pin_nozzle_TriCoil (Pa)"]].to_numpy()
# rnd_un_Pa_atm = df_2[["Atm Pressure (Pa)"]].to_numpy()
# rnd_un_T_r_exv_in = df_2[["T_exv_in (K)"]].to_numpy()
# rnd_un_P_r_exv_in = df_2[["P_exv_in (Pa)"]].to_numpy()
# rnd_un_T_r_evap_out = df_2[["T_evap_out (K)"]].to_numpy()
# rnd_un_P_r_evap_out = df_2[["P_evap_out (Pa)"]].to_numpy()
# rnd_un_m_dot_r = df_2[["m_dot_r (kg/s)"]].to_numpy()
# rnd_un_T_w_in = df_2[["T_w_in (K)"]].to_numpy()
# rnd_un_T_w_out = df_2[["T_w_out (K)"]].to_numpy()
# rnd_un_P_w_in = df_2[["P_w_in (Pa)"]].to_numpy()
# rnd_un_dp_w = df_2[["dp_w (K)"]].to_numpy()
# rnd_un_m_dot_w = df_2[["m_dot_w (kg/s)"]].to_numpy()
...
V_dot_a = air_flow(dp_a_avg, un_dp_a, P_atm_avg, un_Pa_atm, T_db_in_avg, un_T_db_in, T_wb_in_avg, un_T_wb_in, ND, un_ND)
...
files = result_file_loc + "/results.csv"
with open(files, 'a') as f_object:
