...
for filename in files:
    ...
    T_db_out = df[["Tdb_out (C)"]].to_numpy()
    T_db_out += 273.15 #C to K
    
    T_tc_out = df[["TriCoil Outlet (Avg)"]].to_numpy()
    T_tc_out += 273.15 #C to K
    ...
    rnd_un_T_tc_out = 2 * np.std(T_tc_out)
    ...
    sys_un_T_db_in = 0.06 + 0.001*(T_db_in_avg - 273.15) #K
    sys_un_T_wb_in = 0.06 + 0.001*(T_wb_in_avg - 273.15) #K
    sys_un_T_db_out = 0.06 + 0.001*(T_db_out_avg - 273.15) #K
    sys_un_T_tc_out = 0.36 #K
    sys_un_T_wb_out = 0.06 + 0.001*(T_wb_out_avg - 273.15) #K
    sys_un_dp_a = 1.8663 #Pa
    sys_un_Pa_in = 6.221 #Pa
    sys_un_Pa_atm = 0.001 #Pa
    #sys_un_T_r_exv_in = 0.25 + 0.0042*(T_r_exv_in_avg - 273.15) #K
    sys_un_T_r_exv_in = 0.12 #K
    sys_un_P_r_exv_in =4481.594 #Pa
    #sys_un_T_r_evap_out = 0.25 + 0.0042*(T_r_evap_out_avg - 273.15) #K
    sys_un_T_r_evap_out = 0.09 #K
    sys_un_P_r_evap_out = 4481.594 #Pa
    sys_un_m_dot_r = 0.001 * m_dot_r_avg #kg/s
    # sys_un_T_w_in = 0.25 + 0.0042*(T_w_in_avg - 273.15)
    # sys_un_T_w_out = 0.25 + 0.0042*(T_w_out_avg - 273.15)
    sys_un_T_w_in = 0.07 #K
    sys_un_T_w_out = 0.08 #K
    ...
    un_T_tc_out = math.sqrt(sys_un_T_tc_out**2 + rnd_un_T_tc_out**2)#K
    ...
    w_a_out = HAPropsSI('W', 'T', T_db_out_avg, 'B', T_wb_out_avg, 'P', P_atm_avg)
    h_a_out = HAPropsSI('H', 'T', T_tc_out_avg, 'W', w_a_out, 'P', P_atm_avg) #Need to check
    ...
    labels = ['{0} - {1:1.3f} %'.format(i,j) for i,j in zip(labels, Contr_Q_a)]
    ...
    labels=['$T_{db_{in}}$', '$T_{db_{out},TC}$', '$NDD$', '$mu$', '$rho$', '$delta_P$']
    sizes=Contr_Q_a_s
    patches, texts = ax10.pie(sizes, shadow=False, startangle=90)
    
    labels = ['{0} - {1:1.3f} %'.format(i,j) for i,j in zip(labels, Contr_Q_a_s)]
    ...
    print("Capacity of Air in kW: ", Q_air/1000)
    print("Sensible Capacity of Air in kW: ", Q_air_sens/1000)
    print("Capacity of Water in kW: ", Q_water/1000)
    print("Capacity of Refrigerant in kW: ", Q_refr/1000)
    ...
    a_w_heat_bal = (abs(Q_air_sens) - abs(Q_water))*100/(0.5*(abs(Q_air_sens) + abs(Q_water)))
    a_r_heat_bal = (abs(Q_air_sens) - abs(Q_refr))*100/(0.5*(abs(Q_air_sens) + abs(Q_refr)))
    a_r_heat_bal_act = (abs(Q_air) - abs(Q_refr))*100/(0.5*(abs(Q_air) + abs(Q_refr)))
    w_r_heat_bal = (abs(Q_water) - abs(Q_refr))*100/(0.5*(abs(Q_water) + abs(Q_refr)))
    a_w_r_heat_bal = ((abs(Q_air_sens) + abs(Q_water)) - abs(Q_refr))*100/(0.5*((abs(Q_air_sens) + abs(Q_water)) + abs(Q_refr)))
    
    print("Air-Water heat balance: ", a_w_heat_bal)
    print("Air-Refr heat balance: ", a_r_heat_bal)
    print("Actual Air-Refr heat balance: ", a_r_heat_bal_act)
    ...
    with open(result_files, 'a') as f_object:
        ...
        List = [Test_name, 
                ...
                m_dot_r_avg*3600, 
                Q_air.nominal_value/1000,
                Q_air.s/1000,
                Q_air_sens.nominal_value/1000,
                Q_air_sens.s/1000,
                Q_water.nominal_value/1000,
                Q_water.s/1000,
                Q_refr.nominal_value/1000,
                Q_refr.s/1000,
                a_w_heat_bal.nominal_value, 
                a_r_heat_bal.nominal_value, 
                w_r_heat_bal.nominal_value,
                a_w_r_heat_bal.nominal_value]
        writer_object.writerow(List)
        # Close the file object
        f_object.close()

    air_result_files = result_file_loc + "/results_air.csv"
    with open(air_result_files, 'a') as f_object:
    ...
    water_result_files = result_file_loc + "/results_water.csv"
    with open(water_result_files, 'a') as f_object:
    ...
    refr_result_files = result_file_loc + "/results_refr.csv"
    with open(refr_result_files, 'a') as f_object:
    ...
    uncer_files = result_file_loc + "/uncertainties.csv"
    with open(uncer_files, 'a') as f_object:
        ...
        List = [Test_name, 
                ...
                rnd_un_T_w_in,
                un_T_w_in,
                sys_un_P_w_in/6894.76,
                rnd_un_P_w_in/6894.76,
                un_P_w_in/6894.76,
                sys_un_T_w_out,
                rnd_un_T_w_out,
                un_T_w_out,
                ...
                rnd_un_T_r_exv_in,
                un_T_r_exv_in,
                sys_un_P_r_exv_in/6894.76,
                rnd_un_P_r_exv_in/6894.76,
                un_P_r_exv_in/6894.76,
                sys_un_T_r_evap_out,
                rnd_un_T_r_evap_out,
                un_T_r_evap_out,
                sys_un_P_r_evap_out/6894.76,
                rnd_un_P_r_evap_out/6894.76,
                un_P_r_evap_out/6894.76,
                sys_un_m_dot_r*3600,
                rnd_un_m_dot_r*3600,
                un_m_dot_r*3600,
                Q_air.s/1000, 
    ...
    air_uncer_files = result_file_loc + "/uncertainties_air.csv"
    with open(air_uncer_files, 'a') as f_object:
        ...
        List = [Test_name, 
                ...
                Q_air.s/1000, 
                Q_air_sens.s/1000,
                Q_air.s*100/Q_air.nominal_value,
                Q_air_sens.s*100/Q_air_sens.nominal_value]
        writer_object.writerow(List)
        # Close the file ob
        f_object.close()
            
    water_uncer_files = result_file_loc + "/uncertainties_water.csv"
    with open(water_uncer_files, 'a') as f_object:
        ...
        List = [Test_name, 
                ...
                un_m_dot_w*3600,
                Q_water.s/1000,
                Q_water.s*100/Q_water.nominal_value,]
        writer_object.writerow(List)
        # Close the file ob
        f_object.close()
            
    refr_uncer_files = result_file_loc + "/uncertainties_refr.csv"
    with open(refr_uncer_files, 'a') as f_object:
        ...
        List = [Test_name, 
                ...
                un_m_dot_r*3600,
                Q_refr.s/1000,
                Q_refr.s*100/Q_refr.nominal_value]
        writer_object.writerow(List)
        # Close the file ob
        f_object.close()
    
    hb_files = result_file_loc + "/heat_balance.csv"
    with open(hb_files, 'a') as f_object:
